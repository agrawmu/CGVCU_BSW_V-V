...
*** Test Cases ***
Send mAdcuFaults with Signal Values and Verify by reading XCP Variable
...
Send mAdcuStat with Signal Values and Verify by reading XCP Variable
    [Documentation]    Validate RX message and XCP variable read
...
Send mExtLightsADRq with Signal Values and Verify by reading XCP Variable
...
Send mVehCtrlADRq with Signal Values and Verify by reading XCP Variable
    [Documentation]    Validate RX message and XCP variable read

    Log    Assigning Minimum Value to the signal
    ${signals}=    Evaluate    {"VehSpdCmd": -64, "VehAccelLongCmd": -32, "VehRoadWhlAngleCmd": -327.67, "b_High_Deceleration_Flag": 0, "AebEvent": 0, "AebSrc": 0, "MsgCntr": 0, "VehDirCmd": 0, "MsgCrc": 0}
    Run Keyword And Continue On Failure    Send Can Message    mVehCtrlADRq    ${signals}
    Sleep    1s
    ${RawVehSpdCmd}=    Evaluate    int(-64 / 0.015625)
    ${RawVehAccelLongCmd}=    Evaluate    int(-32 / 0.015625)
    ${RawVehRoadWhlAngleCmd}=    Evaluate    int(-327.67 / 0.01)
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehSpdCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehSpdCmd}    -64
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehAccelLongCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehAccelLongCmd}    -32
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehRoadWhlAngleCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehRoadWhlAngleCmd}    -327.67    precision=0.001
    ...
    Run Keyword And Continue On Failure    Send Can Message    mVehCtrlADRq    ${signals}
    Sleep    1s
    ${RawVehSpdCmd}=    Evaluate    int(0 / 0.015625)
    ${RawVehAccelLongCmd}=    Evaluate    int(0 / 0.015625)
    ${RawVehRoadWhlAngleCmd}=    Evaluate    int(0 / 0.01)
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehSpdCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehSpdCmd}    0    precision=0.001
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehAccelLongCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehAccelLongCmd}    0    precision=0.001
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehRoadWhlAngleCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehRoadWhlAngleCmd}    0    precision=0.001
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.b_High_Deceleration_Flag
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${value}    1
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.AebEvent
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${value}    1
    ...
    Run Keyword And Continue On Failure    Send Can Message    mVehCtrlADRq    ${signals}
    Sleep    1s
    ${RawVehSpdCmd}=    Evaluate    int(63.96875 / 0.015625)
    ${RawVehAccelLongCmd}=    Evaluate    int(31.984375 / 0.015625)
    ${RawVehRoadWhlAngleCmd}=    Evaluate    int(327.67 / 0.01)
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehSpdCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehSpdCmd}    63.96875    precision=0.001
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehAccelLongCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehAccelLongCmd}    31.984375    precision=0.001
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.VehRoadWhlAngleCmd
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${RawVehRoadWhlAngleCmd}    327.67    precision=0.001
    ${value}=    Read Xcp Variable    rtARID_DEF_CpApAdcuRx20.PP_SG_mVehCtrlADRq_VDP_SG_mVehC.b_High_Deceleration_Flag
    Run Keyword And Continue On Failure    Should Be Equal As Numbers    ${value}    1
